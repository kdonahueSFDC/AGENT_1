...
export default class UserStoryKanban extends NavigationMixin(LightningElement) {
  ...
  wiredFeatures({ error, data }) {
    if (data) {
      ...
      this.showErrorToast(
        ...
        error.body?.message || "Failed to load features."
      );
    }
  }
  ...
  wiredStoriesByStatus(result) {
    this.wiredStoriesResult = result;
    ...
    if (data) {
      ...
      this.columnsWithStories = STATUS_COLUMNS.map((column) => {
        const stories = (data[column.value] || []).map((story) => ({
          ...
          moveToOptions: STATUS_COLUMNS.filter(
            (opt) => opt.value !== story.status
          ).map((opt) => ({
      ...
      this.showErrorToast(
        "Error Loading User Stories",
        error.body?.message || "Failed to load user stories."
      );
    }
  }
  ...
  handleMoveStory(event) {
    event.stopPropagation();
    const newStatus = event.detail.value;
    const storyId = event.currentTarget.dataset.storyId;
    const oldStatus = event.currentTarget.dataset.storyStatus;

    if (storyId && newStatus && oldStatus && newStatus !== oldStatus) {
      this.updateStoryStatus(storyId, oldStatus, newStatus);
    }
  }

  updateStoryStatus(storyId, oldStatus, newStatus) {
    if (this.isUpdating) {
      return;
    }
    ...
    if (story) {
      ...
      this.storiesByStatus[newStatus] = [
        ...(this.storiesByStatus[newStatus] || []),
        updatedStory
      ];
    ...
    updateUserStoryStatus({ storyId: storyId, newStatus: newStatus })
      .then((updatedStory) => {
        ...
        this.showSuccessToast(
          "Status Updated",
          `User story "${updatedStory.name}" moved to ${this.getStatusLabel(newStatus)}.`
        );
      ...
      .catch((error) => {
        ...
        this.showErrorToast(
          "Update Failed",
          error.body?.message || "Failed to update user story status."
        );
  ...
  refreshData() {
    if (this.wiredStoriesResult) {
      return refreshApex(this.wiredStoriesResult);
    }
